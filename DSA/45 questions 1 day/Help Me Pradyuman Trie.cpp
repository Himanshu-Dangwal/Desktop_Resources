...
void insert(trieNode* head , string s){
    if(s.empty()){
        head->isEndOfTrie = true;
        return;
    }
    trieNode* curr_node;
    int index = s[0]-'a';
    if(!head->store[index]) {
        head->store[index] = new trieNode();
    }
    curr_node = head->store[index];
    insert(curr_node,s.substr(1));
}

void display(trieNode* head , string s , int level) {
    if(head->isEndOfTrie) {
        cout  << s << endl;
    }
    for(int i=0 ; i<26 ; i++){
        if(head->store[i]){
            s.push_back(i + 'a');
            display(head->store[i],s,level+1);
            s.pop_back();
        }
    }
}
